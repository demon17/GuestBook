...
class Twig_Tests_ErrorTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testTwigExceptionAddsFileAndLineWhenMissingWithInheritanceOnDisk()
    {
        ...
        $template = $twig->loadTemplate('show_news.html');
        try {
            $template->render(array());

            $this->fail();
        } catch (Twig_Error_Runtime $e) {
            $this->assertEquals('Variable "foo" does not exist in "show_news.html" at line 3', $e->getMessage());
            $this->assertEquals(3, $e->getTemplateLine());
            $this->assertEquals('show_news.html', $e->getTemplateFile());
        }
        ...
        try {
            ...
            $this->assertEquals('An exception has been thrown during the rendering of a template ("Runtime error...") in "show_news.html" at line 3.', $e->getMessage());
            $this->assertEquals(3, $e->getTemplateLine());
            $this->assertEquals('show_news.html', $e->getTemplateFile());
        }
    }
